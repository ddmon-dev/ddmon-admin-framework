...
INSERT INTO public.inquiry_replies (inquiry_id, content, author, sent_at, created_at) VALUES
  ('a0000000-0000-0000-0000-000000000001', 'API 호출 제한은 Basic 요금제 기준 분당 60회, Pro 요금제 기준 분당 300회입니다. 대량 데이터 처리를 위한 Batch API는 Pro 요금제 이상에서 제공되며, 한 번에 최대 500건까지 처리 가능합니다. 자세한 내용은 개발자 문서(docs.example.com/api)를 참고해주세요.', '관리자', NOW() - INTERVAL '13 days', NOW() - INTERVAL '13 days'),
  ('a0000000-0000-0000-0000-000000000002', '간편결제 도입은 현재 개발 로드맵에 포함되어 있으며, 2024년 2분기 중 카카오페이와 네이버페이를 우선 지원할 예정입니다. 토스페이 등 추가 결제 수단도 순차적으로 도입할 계획입니다. 업데이트 시 별도 안내드리겠습니다.', '관리자', NOW() - INTERVAL '11 days', NOW() - INTERVAL '11 days'),
  ('a0000000-0000-0000-0000-000000000003', '회원 탈퇴 시 개인정보는 관련 법령에 따라 30일간 보관 후 완전 삭제됩니다. 즉시 삭제를 원하시는 경우 고객센터로 별도 요청해주시면 수동 처리가 가능합니다. 단, 전자상거래법에 따라 거래 기록은 5년간 보관됩니다.', '관리자', NOW() - INTERVAL '9 days', NOW() - INTERVAL '9 days'),
  ('a0000000-0000-0000-0000-000000000008', '기존 데이터는 업그레이드 시 100% 유지됩니다. 요금은 남은 기간에 대해 일할 계산되며, Basic 잔여 금액을 차감한 Pro 요금이 청구됩니다. 예를 들어 15일 남은 시점에서 업그레이드하시면 Basic 15일분을 공제한 차액만 결제하시면 됩니다.', '관리자', NOW() - INTERVAL '3 days', NOW() - INTERVAL '3 days'),
  ('a0000000-0000-0000-0000-000000000012', '현재 Tiptap 기반 에디터에서 유튜브, Vimeo 등 동영상 임베드를 지원하고 있습니다. 에디터 상단 툴바의 "미디어 삽입" 아이콘을 클릭하시면 URL을 입력하여 임베드할 수 있습니다. 혹시 해당 아이콘이 보이지 않으시면 브라우저 캐시를 삭제 후 다시 시도해주세요.', '관리자', NOW() - INTERVAL '18 hours', NOW() - INTERVAL '18 hours');
...
INSERT INTO public.popups (title, content, author, position_top, position_left, width, is_active, is_always, start_date, end_date, z_index) VALUES
  ('신년 이벤트 안내', '<div style="text-align:center;padding:20px;"><h2 style="color:#e74c3c;margin-bottom:12px;">2024 신년 특별 이벤트</h2><p>신규 가입 시 <strong>30% 할인 쿠폰</strong>을 드립니다!</p><p style="margin-top:16px;color:#666;">기간: 2024.01.01 ~ 2024.01.31</p></div>', '마케팅팀', 100, 100, 450, true, false, '2024-01-01', '2024-01-31', 10),
  ('시스템 점검 안내', '<div style="padding:20px;"><h3 style="margin-bottom:10px;">시스템 정기 점검 안내</h3><p>아래 시간 동안 서비스 이용이 제한됩니다.</p><ul style="margin-top:10px;padding-left:20px;"><li>일시: 2024.02.15 (목) 02:00 ~ 06:00</li><li>대상: 전체 서비스</li></ul><p style="margin-top:10px;color:#888;">이용에 불편을 드려 죄송합니다.</p></div>', 'IT운영팀', 150, 200, 400, false, false, '2024-02-14', '2024-02-15', 20),
  ('개인정보 처리방침 변경', '<div style="padding:20px;"><h3>개인정보 처리방침 변경 안내</h3><p style="margin-top:10px;">2024년 3월 1일부로 개인정보 처리방침이 변경됩니다.</p><p style="margin-top:8px;">주요 변경 사항:</p><ol style="padding-left:20px;margin-top:8px;"><li>수집 항목 변경</li><li>보관 기간 조정</li><li>제3자 제공 범위 변경</li></ol></div>', '법무팀', 80, 150, 420, true, false, '2024-02-15', '2024-03-15', 15),
  ('긴급 공지', '<div style="padding:20px;text-align:center;"><h2 style="color:#c0392b;">긴급 공지</h2><p style="margin-top:12px;">현재 결제 시스템 일시 장애가 발생하여<br/>복구 작업을 진행 중입니다.</p><p style="margin-top:16px;font-weight:bold;">예상 복구 시간: 1시간 이내</p></div>', 'IT운영팀', 120, 250, 380, false, false, NULL, NULL, 30),
  ('프로모션 배너', '<div style="padding:24px;text-align:center;background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);color:#fff;border-radius:8px;"><h2 style="margin-bottom:8px;">Premium Plan</h2><p>지금 업그레이드하면 첫 3개월 50% 할인!</p><p style="margin-top:12px;font-size:24px;font-weight:bold;">월 29,900원 → 14,950원</p></div>', '마케팅팀', 200, 300, 500, true, true, NULL, NULL, 5),
  ('설문조사 참여 요청', '<div style="padding:20px;"><h3>고객 만족도 설문조사</h3><p style="margin-top:10px;">서비스 개선을 위한 설문조사에 참여해주세요.</p><p style="margin-top:8px;">소요 시간: 약 3분</p><p style="margin-top:12px;color:#3498db;"><strong>참여하신 분들께 스타벅스 기프티콘을 드립니다!</strong></p></div>', 'CS팀', 100, 50, 380, true, false, '2024-03-01', '2024-03-31', 10),
  ('앱 업데이트 안내', '<div style="padding:20px;text-align:center;"><h3>모바일 앱 v3.0 업데이트</h3><p style="margin-top:10px;">새로워진 기능을 만나보세요!</p><ul style="text-align:left;margin-top:12px;padding-left:20px;"><li>다크 모드 지원</li><li>푸시 알림 개선</li><li>성능 최적화</li></ul></div>', '개발팀', 80, 120, 360, false, false, '2024-03-10', '2024-04-10', 10),
  ('채용 공고', '<div style="padding:20px;"><h3 style="color:#2c3e50;">함께 성장할 인재를 찾습니다</h3><p style="margin-top:10px;">프론트엔드 개발자, 백엔드 개발자, 디자이너를 모집합니다.</p><p style="margin-top:12px;">지원 마감: 2024.04.30</p><p style="margin-top:8px;color:#3498db;">자세한 내용은 채용 페이지를 확인해주세요.</p></div>', '인사팀', 160, 180, 420, true, false, '2024-02-01', '2024-04-30', 8);
