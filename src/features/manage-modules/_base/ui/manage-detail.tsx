...
export function DetailField({
  ...
  return (
    <div className={cn('grid grid-cols-[6rem_1fr]', className)}>
      <span className={cn('text-sm font-medium bg-muted/50 px-4 py-2.5 flex', labelClassName)}>
        {label}
      </span>
      <span className={cn('text-sm px-4 py-2.5', valueClassName)}>{displayValue}</span>
    </div>
  );
}
...
export function DetailContainer({ children, className }: DetailContainerProps) {
  return <div className={cn('space-y-8 pb-6 md:pb-8', className)}>{children}</div>;
}
