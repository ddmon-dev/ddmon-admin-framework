...
describe('updateItem', () => {
  ...
  it('schema 있을 때 부분 필드 → partial()로 성공', async () => {
    const mockData = { id: '1', title: '부분 수정', updated_by: 'admin1' };
    mockSingle.mockResolvedValue({ data: mockData, error: null });
    ...
    expect(result).toEqual({ success: true, data: mockData });
    expect(mockUpdate).toHaveBeenCalledWith(
      expect.objectContaining({ title: '부분 수정', updated_by: 'admin1' })
    );
  });

  it('schema 없을 때 → 검증 스킵, DB update 진행', async () => {
    const mockData = { id: '2', title: '아무값', updated_by: 'admin1' };
    mockSingle.mockResolvedValue({ data: mockData, error: null });
    ...
    expect(result).toEqual({ success: true, data: mockData });
    expect(mockUpdate).toHaveBeenCalledWith(
      expect.objectContaining({ title: '아무값', updated_by: 'admin1' })
    );
  });

  it('pathname 있을 때 → revalidatePath 호출', async () => {
    const mockData = { id: '3', title: '경로', updated_by: 'admin1' };
    mockSingle.mockResolvedValue({ data: mockData, error: null });
  ...
  it('cleanupDeletedFiles 호출 확인', async () => {
    const mockData = { id: '4', title: '파일 정리', updated_by: 'admin1' };
    mockSingle.mockResolvedValue({ data: mockData, error: null });
